...
(function () {
  ...
  /* The enquiry form posts to the Digi2U Website Inquiries Google Form, which
     appends a row to its sheet and mails the team. It used to hand the message
     to the visitor's own mail client, which meant a lead only arrived if that
     person had a mail client configured and actually pressed send in it — the
     mailto is now the fallback for a failed POST, not the default path. */
  D2UForms.bind({
    form: 'inquiryForm',
    note: 'f-note',
    collect: function () {
